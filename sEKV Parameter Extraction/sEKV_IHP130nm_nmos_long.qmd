  ...
  pdf:
    ...
    include-in-header:
      text: |
        ...
        \cohead[nMOS (long-channel)]{nMOS (long-channel)}
...
\begin{align}
  ...
  \Delta W &= 2\,WOT.
\end{align}
The length and width reduction $\Delta L$ and $\Delta W$ are therefore constant and independent of $L$ and $W$. Their values are given in @tbl-length_width_corrections. 
...
# Channel-width and length reduction for pMOS extracted from sg13g2_moslv_parm.lib
DLPS=lvaro*(1+lvarl*LEN/L)*(1+lvarw*WEN/W)
DWOD=wvaro*(1+wvarl*LEN/L)*(1+wvarw*WEN/W)
DLn=2*lap-DLPS
DWn=2*wot-DWOD
...
sekv_geom_param_df={
    "Width correction DW": [DWn/1e-9],
    "Length correction DL": [DLn/1e-9],
...
Note that for `{python} f'{tYPE}'` $\Delta W =$ `{python} f'{DWn/1e-9:.3f}'` $nm$ is negative wich leads to an effective width that is longer than the drawn width. On the other hand $\Delta L =$ `{python} f'{DLn/1e-9:.3f}'` $nm$ is positive which results in an effective length that is smaller than the drawn length.

The width and length for the selected `{python} f'{tYPE}'` transistor are given by $W =$ `{python} f'{W/1e-6:.0f}'` $\mu m$ and $L =$ `{python} f'{L/1e-6:.0f}'` $\mu m$ and the effective width and length are given by $W_{{eff}} =$ `{python} f'{Weff/1e-6:.3f}'` $\mu m$ and $L_{eff} =$ `{python} f'{Leff/1e-6:.3f}'` $\mu m$. They are summarized in @tbl-mos_geometry.
...
plt.text(0.45, 0.05, textstr, ha='left', va='bottom', size=9, transform=plt.gca().transAxes,
         bbox={'facecolor':'w', 'alpha':1.0, 'pad':5})
...
The fit is now much better in moderate inversion but less in strong inversion. This is due to mobility reduction due to the vertical field an effect that is not accounted for in the model. However, we will keep the new value of $I_{spec\Box}$.
...
The average value of $V_{T0}$ in this range is given by $V_{T0} =$ `{python} f'{VT0wi/1e-3:.0f}'` $mV$. The threshold voltage for this wide and long device is surprisingly small. It is however in accordance with the IHP documentation giving a typical-typical $V_{TH} \cong 200\,mV$ for $W=10\,\mu m$ and $L=10\,\mu m$ @bib:ihp:2025.
...
We get a reasonable fit and parameter values that are similar to what we got from the direct extraction. The weak inversion asymptote is slightly to low. We can correct this by reusing the value of $n$ extracted from the direct extraction above and optimize for $I_{spec}$ only. This reults in @fig-ispec_ext_opt_lc0.

```{python}
#| label: fig-ispec_ext_opt_lc0
#| fig-cap: Extraction of $I_{spec}$ by curve fitting keeping the value of $n$ extracted from the direct extraction.
...
ax3.set_xticks(np.arange(-0.4,1.6,0.2))
ax3.set_ylabel('$I_D/I_{spec}$ (lin)')
ax3.set_ylim(0,500)
...
We now will extract the output conductance parameter, namely the Early voltage $V_E$, $\lambda$ and the intercept current $I_{D0}$ using linear regression.
...
The linear regression approximation of the drain current in saturation is shown in @fig-clm_param_ext by the dashed black line. We get a rather high output conductance $G_{ds} =$ `{python} f'{Gdsext/1e-9:.1f}'` $nA/V$ and hence low value of the $\lambda$ parameter with $\lambda =$ `{python} f'{lambdaext/1e6:.3f}'` $V/\mu m$ for such a long transistor! This translates into a poor intrinsic voltage gain $G_m/G_{ds}$. The output conductance parameters are summarized in @tbl-clm_parameters1.
...
Markdown(tabulate(
  ...
  floatfmt=(".0f",".3f",".3f",".1f",".3f",".3f",".3f",".0f")
))
```

# Noise
In this section we will extract the flicker noise parameters to be used with sEKV and check the white noise power spectral density (PSD). We reuse the flicker noise model from EKV 2.6, where the input (gate) referred PSD is given by @bib:bucher:ekv2p6:1998
\begin{equation}
  S_{nin,fl}(f) = \frac{KF}{W_{eff}\,L_{eff}\,C_{ox}\; f^{AF}}
\end{equation}
In this model the flicker noise is assumed to scale as $1/C_{ox}$, which is correct if the noise follows the the Hooge model (i.e. originates from mobility fluctuations). In the case of the Mc Worther model (i.e. flicker noise originating from traps in Si-SiO~2~ interface and in the oxyde), the PSD scales as $C_{ox}^2$. Despite the flicker noise is usually dominated by the trapping mechanism, we will keep the above model with a $1/C_{ox}$ scaling.

In EKV , we like to rewrite the flicker noise PSD like the thermal noise in terms of a input-referred noise resistance defnied as
\begin{equation}
...
Having extracted $n$, $I_{spec\Box}$ and $V_{T0}$, we can impose the inversion coefficient and calculate the corresponding gate voltage $V_G$.
...
mosop_df=mosop_df.rename(columns={'@n.xn.nsg13_lv_nmos[weff]': 'Transistor',
                              '@n.xn.Nsg13_lv_nmos[weff]': 'Weff',
                              '@n.xn.Nsg13_lv_nmos[leff]': 'Leff',
                              '@n.xn.Nsg13_lv_nmos[ids]': 'IDS',
                              '@n.xn.Nsg13_lv_nmos[gm]': 'Gm',
                              '@n.xn.Nsg13_lv_nmos[gds]': 'Gds',
                              '@n.xn.Nsg13_lv_nmos[sid]': 'Snidth',
                              '@n.xn.Nsg13_lv_nmos[sqrtsfw]': 'Vninth',
                              '@n.xn.Nsg13_lv_nmos[sfl]': 'Snidfl @ 1Hz',
                              '@n.xn.Nsg13_lv_nmos[sqrtsff]': 'Vninfl @ 1kHz',
                              '@n.xn.Nsg13_lv_nmos[fknee]': 'fk'
                    })
...
pspop_df={
    ...
    "Leff": [mosop_df.at['Mn','Leff']/1e-6],
    "IDS": [mosop_df.at['Mn','IDS']/1e-9],
...
We see that the sEKV estimation is very close to the simulated white noise with PSP. We also obsserve that the sEKV input-referred flicker noise estimation is right on top of PSP. The extracted sEKV flicker noise parameters are given in @tbl-flicker_noise_parameters.
...
```{python}
with pd.ExcelWriter("sEKV_ihp130nm_nmos_long.xlsx") as writer:
    sekv_geom_param_df.to_excel(writer, sheet_name="geometric parameters (nMOS)")
    sekv_idvg_param_df.to_excel(writer, sheet_name="idvg parameters (nMOS)")
    sekv_idvd_param_df.to_excel(writer, sheet_name="idvd parameters (nMOS)")
    sekv_noise_param_df.to_excel(writer, sheet_name="noise parameters (nMOS)")
```
...
The sEKV parameters were first extracted using a direct extraction methodology with $\lambda_c =0$. This results in a good fit over all regions of operation except in very strong inversion because effect of mobility reduction due to the vertical field is not included in the sEKV model.

Things can be improved by using the $\lambda_c$ parameter to account for this effect although this is a very long-channel transistor.

We also have checked that the thermal noise of EKV matches the PSP model and we have extracted the flicker noise parameters.

Overall we have shown that sEKV can fit the large- and small-signal data of this is a `{python} f'{size}'`-channel  `{python} f'{tYPE}'` transistor very well and that the noise is consistent with PSP.
